...
struct yautof
{
	static rbool auto_add_func(const tsh& sh,tclass& tci)
	{
		...
		if(yfind::func_search(tci,rppoptr(c_equal),tci.name+rppoptr(c_addr),
		...
		if(yfind::func_search(tci,rppoptr(c_equal),
		...
		if(yfind::func_search(tci,rppoptr(c_addr))==null)
		{
			tfunc tfi;
			tfi.ptci=&tci;
			tfi.first_pos=pos;
			...
			if(sh.m_mode==tsh::c_gpp)
			{
				...
				ybase::push_twi(tfi.vword,twi,rppkey(c_this));
				ybase::push_twi(tfi.vword,twi,";");
			}
			else
			{
				ybase::push_twi(tfi.vword,twi,rppkey(c_mov));
				ybase::push_twi(tfi.vword,twi,rppkey(c_s_ret));
				ybase::push_twi(tfi.vword,twi,rppoptr(c_comma));
				ybase::push_twi(tfi.vword,twi,rppkey(c_this));
			}
			tfi.name_dec=tfi.get_dec();
	...
	static void add_copy_sent(const tsh& sh,tfunc& tfi)
	{
		...
		for(int i=0;i<tci.vdata.count();i++)
		{
			...
			ybase::push_twi(tfi.vword,twi,rppkey(c_this));
			ybase::push_twi(tfi.vword,twi,rppoptr(c_dot));
			ybase::push_twi(tfi.vword,twi,tci.vdata[i].name);
			ybase::push_twi(tfi.vword,twi,rppoptr(c_equal));
			ybase::push_twi(tfi.vword,twi,rppkey(c_rvara));
			ybase::push_twi(tfi.vword,twi,rppoptr(c_dot));
			ybase::push_twi(tfi.vword,twi,tci.vdata[i].name);
			ybase::push_twi(tfi.vword,twi,rppoptr(c_semi));
		}
	}
};
