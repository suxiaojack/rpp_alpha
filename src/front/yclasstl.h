...
struct yclasstl
{
	...
	static rbool type_replace(tsh& sh,rbuf<tword>& v,int level=0)
	{
		...
		for(int i=1;i<v.count();i++)
		{
			...
			{
				rpperror(v[i-1],"template not match");
				return false;
			}
			...
			if(ptci==null)
			{
				rpperror(v.get(i),"can't find class "+item.name);
				return false;
			}
		...
		combine_quote(sh,v);
		ybase::arrange_format_l(sh,v);//匿名函数要在这里处理大括号换行
	...
	static rbool point_replace(const tsh& sh,rbuf<tword>& v)
	{
		for(int i=0;i<v.count();i++)
		{
			if(v.get(i+1).val!=rppoptr(c_mul)||
				!yfind::is_class(sh,v[i].val))
			{
				continue;
			}
			if(v.get(i+2).val==rppoptr(c_mul))
			{
				...
				ybase::push_twi(vword,word,rppkey(c_rp));
				ybase::push_twi(vword,word,rppoptr(c_tbk_l));
				ybase::push_twi(vword,word,rppkey(c_rp));
				ybase::push_twi(vword,word,rppoptr(c_tbk_l));
				ybase::push_twi(vword,word,v[i].val);
				ybase::push_twi(vword,word,rppoptr(c_tbk_r));
				ybase::push_twi(vword,word,rppoptr(c_tbk_r));
