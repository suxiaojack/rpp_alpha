...
struct yexp
{
		...
		tfunc& tfi,int level,int& i,tenv env)
	{
		...
		if(ptci==null)
		{
			rpperror(src,"can't find class name");
			return false;
		}
		i+=2;
		...
		tsent& outopnd,tfunc& tfi,int level,tenv env)
	{
		...
		if(ptci==null)
		{
			rpperror(first,"can't find func mbk");
			return false;
		}
		...
		tfunc& tfi,int level,int& i,const tclass* ptci,tenv env)
	{
		...
		if(null==ptfi)
		{
			rpperror(src,"p_mbk_param can't find call");
			return false;
		}
		rbuf<tsent> vsent;
		ybase::split_param(sh,vsent,src.vword.sub(left+1,right),src);
		...
		tfunc& tfi,int level,int& i,const tclass* ptci,tenv env)
	{
			...
			yfind::is_end_part_optr(sh,src.vword.get(i+1).val))
		{
			...
			if(ptfi==null)
			{
				rpperror(src,"p_call_n can't find call");
				return false;
			}
		}
		else
		{
			...
			if(ptfi==null)
			{
				rpperror(src,"p_call_n2 can't find call");
				return false;
			}
		}
		...
		tfunc& tfi,int level,int& i,const tclass* ptci,tenv env)
	{
		...
		if(pfirst!=null&&ybase::get_tname(pfirst->type)==rppkey(c_var)&&
		...
		if(null==ptfi)
		{
			if(pfirst==null)
			{
				rpperror(src,"p_call can't find call");
				return false;
			}
			vsent.pop_front();
			ptfi=ymatch::find_replace(sh,*ptci,fname,vsent);
		...
		int level,tenv env)
	{
		...
		if(level>c_rpp_deep)
		{
			rpperror(src,"expression too deep");
			return false;
		}
		level++;
		dst.clear();
		rbuf<rstr> soptr;
		rbuf<tsent> sopnd;
		soptr.push(rppoptr(c_sharp_sharp));
		src.vword.push(tword(rppoptr(c_sharp_sharp)));
		for(int i=0;i<src.vword.count();++i)
		{
			if(src.vword[i].val==rppoptr(c_sharp_sharp)&&
			...
			elif(src.vword[i].val==rppoptr(c_arrow_r))
			{
				...
				tsent first=sopnd.pop();
				first.vword.push_front(tword(rppoptr(c_mul)));
