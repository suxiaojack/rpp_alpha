...
struct yfind
{
	...
	static rbool is_type_mebx(const tsh& sh,const rstr& type)
	{
		...
		return type==rppkey(c_int)||ybase::is_point(type)||ybase::is_quote(type);
	}

	static rbool is_op_pass_type(const tsh& sh,const rstr& s)
	{
		return s==rppkey(c_int)||s==rppkey(c_uint)||ybase::is_point(s);
	}

	static rbool is_rstr_optr(const tsh& sh,const rstr& s)
	{
		return (s==rppoptr(c_plus)||s==rppoptr(c_equal_equal)||
			s==rppoptr(c_not_equal)||s==rppoptr(c_great_equal)||
			s==rppoptr(c_less_equal));
