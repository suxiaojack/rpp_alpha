...
struct zinterpret
{
	static void init_addr_list(tsh& sh)
	{
		...
		rppjf("printf",xf::printf);
		rppjf("_vsnprintf",xf::vsnprintf);
		rppjf("sscanf",xf::sscanf);
		rppjf("sprintf",xf::sprintf);
		rppjf("exit",xf::exit);
		rppjf("malloc",xf::malloc);
		rppjf("free",xf::free);
		rppjf("memcpy",xf::memcpy);
		rppjf("strlen",xf::strlen);
		rppjf("system",xf::cmd);
		rppjf("ftell",xf::ftell);
		rppjf("fseek",xf::fseek);
		rppjf("_wfopen",xf::fopenw);
		rppjf("fclose",xf::fclose);
		rppjf("fread",xf::fread);
		rppjf("fwrite",xf::fwrite);
	}
...
next:
	...
	static rbool asm_init(tsh& sh,rbuf<tasm>& vasm)
	{
		...
		vasm+=rf::vstr("call",rppoptr(c_mbk_l),rppoptr(c_addr),
