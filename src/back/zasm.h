...
struct zasm
{
	static rbool process(tsh& sh,tfunc& tfi)
	{
		for(int i=0;i<tfi.vsent.count();i++)
		{
			...
			{
				rpperror(tfi.vsent[i],"build asm error");
				return false;
			}
			if(tconf::c_op_match)
			{
	...
	{
		vasm+=rf::vstr(rppkey(c_push),rstr(*((uint*)(&dval)+1)));
		vasm+=rf::vstr(rppkey(c_push),rstr(*((uint*)(&dval))));
	}
	...
	static rbool add_asm_ebp(const tsh& sh,tfunc& tfi)
	{
		...
		{
			item.vstr=rf::vstr(rppkey(c_sub),rppkey(c_esp),
				rppoptr(c_comma),rstr(size));
			vasm+=item;
		}
		rbool b_use_ebp=use_ebp(sh,tfi.vasm);
		if(b_use_ebp)
		{
			item.vstr=rf::vstr(rppkey(c_push),rppkey(c_ebp));
			vasm+=item;
			item.vstr=rf::vstr(rppkey(c_mov),rppkey(c_ebp),
		...
		if(tfi.is_cfunc)
		{
			...
			item.vstr=rf::vstr(rppkey(c_push),rppkey(c_ebx));
			vasm+=item;
			item.vstr=rf::vstr(rppkey(c_push),rppkey(c_esi));
			vasm+=item;
			item.vstr=rf::vstr(rppkey(c_push),rppkey(c_edi));
			vasm+=item;
		}
		vasm+=tfi.vasm;
		...
		tfunc& tfi,int level=0)
	{
		...
		if(src.vword.count()==1)
		{
			if(src.vword[0].is_cint())
			{
				...
				vasm+=rf::vstr(rppkey(c_mov),rppkey(c_ebx),
		...
		{
			vasm+=rf::vstr(rppkey(c_mov),rppkey(c_ebx),rppoptr(c_comma),
		...
		if(ptdi!=null)
		{
			...
			{
				vasm+=rf::vstr(rppkey(c_lea),rppkey(c_esi),
				...
				if(ybase::is_quote(src.type)&&
					(ybase::get_tname(src.type)==rppkey(c_int)||
		...
		tfunc& tfi,int level)
	{
		...
		if(src.type!=rppkey(c_void)&&yfind::is_type_mebx(sh,src.type))
		{
			vasm+=rf::vstr(rppkey(c_mov),rppkey(c_esi),
				rppoptr(c_comma),rppkey(c_esp));
			int cur=vasm.count();
		...
		vasm+=rf::vstr(rppkey(c_add),rppkey(c_esp),rppoptr(c_comma),size);
		return true;
	}
		...
		tfunc& tfi,int level=0)
	{
		...
		if(src.vword.get(1).val==rppkey(c_pcall))
		{
			...
			size=yfind::get_ceil_space(retval);
			vasm+=rf::vstr(rppkey(c_sub),rppkey(c_esp),
			...
			{
				vasm+=rf::vstr(rppkey(c_call),rppkey(c_ebx));
			}
			else
			{
				return false;
		...
		size=yfind::get_ceil_space(retval);
		vasm+=rf::vstr(rppkey(c_sub),rppkey(c_esp),rppoptr(c_comma),size);
		for(int i=vsent.count()-1;i>=0;i--)
		{
		...
		tfunc& tfi,int level)
	{
			...
			yfind::is_class(sh,src_in.vword.get(1).val))
		{
			...
			//释放栈
			size=yfind::get_ceil_space(retval);
			vasm+=rf::vstr(rppkey(c_add),rppkey(c_esp),
				rppoptr(c_comma),size);
		}
		elif(src.vword[0].is_cint())
		{
			vasm+=rf::vstr(rppkey(c_push),src.vword[0].val);
		}
		elif(src.vword[0].is_cuint())
		{
			vasm+=rf::vstr(rppkey(c_push),get_int(src.vword[0].val));
		}
		...
		else
		{
			...
			if(cur==vasm.count()&&tconf::c_op_pass)
			{
				...
				{
					vasm.m_count-=3;
					vasm+=rf::vstr(rppkey(c_push),rppoptr(c_mbk_l),
						rppkey(c_ebp),rppoptr(c_plus),
						rstr(ptdi->off),rppoptr(c_mbk_r));
					return true;
				}
				...
				if(ybase::is_quote(dst.type))
				{
					...
					vasm+=rf::vstr(rppkey(c_lea),rppkey(c_esi),
						...
						rppkey(c_ebp),rppoptr(c_plus),
						rstr(ptdi->off),rppoptr(c_mbk_r));
					vasm+=rf::vstr(rppkey(c_push),rppkey(c_esi));
					return true;
				}
			}
			//调用拷贝构造函数时还可以优化
		...
		const tdata* ptdi,rbuf<tasm>& vasm)
	{
		...
		vasm+=rf::vstr(rppkey(c_sub),rppkey(c_esp),rppoptr(c_comma),size);
		vasm+=rf::vstr(rppkey(c_mov),rppkey(c_edi),rppoptr(c_comma),
			rppkey(c_esp));
		vasm+=rf::vstr(rppkey(c_lea),rppkey(c_esi),rppoptr(c_comma),
		...
		const tsent& src,rbuf<tasm>& vasm)
	{
		...
		vasm+=rf::vstr(rppkey(c_lea),rppkey(c_edi),rppoptr(c_comma),
			rppoptr(c_mbk_l),rppkey(c_esp),rppoptr(c_plus),
			rstr(size),rppoptr(c_mbk_r));
		vasm+=rf::vstr(rppkey(c_mov),rppkey(c_esi),rppoptr(c_comma),
			rppkey(c_esp));
	...
	static tsent get_src_in(const tsh& sh,const tsent& src)
	{
		...
		while(start+2<src.vword.count()&&
			  src.vword[start].val==rppoptr(c_mbk_l)&&
			  src.vword[start+1].val==rppoptr(c_dot))
		{
			start+=3;
		}
		if(src.vword[start].val==rppoptr(c_mbk_l))
		{
	...
	static rbool add_esi(const tsh& sh,rstr type,rbuf<tasm>& vasm,const tsent& src)
	{
		...
		for(int i=0;i<start;i+=3)
		{
			...
			if(ptdi->off!=0)
			{
				vasm+=rf::vstr(rppkey(c_add),rppkey(c_esi),
		...
		const rstr& dst,rbuf<tasm>& vasm)
	{
		...
		if(ybase::is_point(dst)&&ybase::is_point(src)||
			dst==rppkey(c_rd4)&&yfind::get_type_size(sh,src)==4)
		{
			vasm+=rf::vstr(rppkey(c_mov),rppoptr(c_mbk_l),
		...
		vasm+=rf::vstr(rppkey(c_push),rppkey(c_edi));
	...
	static rstr get_int(const rstr& s)
	{
