...
struct zgpp
{
	static rbool process(tsh& sh)
	{
		...
		head+=("#include \""+rcode::utf8_to_gbk(
			ybase::get_rpp_dir_w().torstr())+"ext/mingw/gpp.h\"\n");
		...
		head+="\n";
		head+=result;
		rstrw name=ybase::get_main_name(sh)+rstr(".cpp");
		rfile file;
		ifn(file.open_n(name,rstr("rw")))
		{
			rpperror("file open error");
			return false;
		}
		ifn(file.write(head.size(),head.begin()))
		{
			rpperror("file write error");
			return false;
		}
		return true;
	}
	...
	{
		return ybase::is_basic_type(sh,s)||yfind::is_empty_struct_type(sh,s)||s=="ushort"||s=="int8";
	}
		...
		rbuf<tfunc*>& vdec,int level=0)
	{
		if(level>300)
		{
			rpperror(tfi,"gpp level overflow");
			return false;
		}
		level++;
		if(tfi.count==1)
		...
		for(int i=0;i<tfi.vsent.count();i++)
		{
			...
			if(tfi.vsent[i].vword.get(1)==rppoptr(c_equal))
			{
				continue;
			}
		...
		result+="{\n";
		result+="	int ebx;\n";
		if(tfi.retval.type!=rppkey(c_void))
		{
			result+="	";
		...
		{
			if(!ybase::is_quote(tfi.local[i].type)&&tfi.local[i].name!=rppkey(c_pmain))
			{
				result+="	";
		...
		for(int i=0;i<tfi.vsent.count();i++)
		{
			if(ybase::is_tag<tword>(tfi.vsent[i].vword))
			{
				vtemp+=rf::vstr(/*symbol+"_"+*/tfi.vsent[i].vword[0].val,tfi.vsent[i].vword[1].val);
		...
		ret_quote_trans(vtemp,tfi);
		ifn(control_trans(tfi,vtemp))
		{
			return false;
		}
	...
	static void ret_quote_trans(rbuf<rbuf<rstr> >& v,tfunc& tfi)
	{
		...
		{
			if(v[i].get_left()=="jmp"&&v[i].get(1)=="_func_end")
			{
				if(v[i-1].count()>2&&v[i-1][0]=="ebx"&&v[i-1][1]=="=")
				{
					v[i-1]=v[i-1].sub(2);
				}
				v[i-1]=rf::vstr(tfi.retval.name,"=","&")+v[i-1];//todo:上一句不一定是原来的返回语句
			}
		}
	}

	static rbool control_trans(tfunc& tfi,rbuf<rbuf<rstr> >& v)
	{
		...
		rbuf<tfunc*>& vdec,int level,const rbuf<rstr>& v)
	{
		...
		if(v.get(1)==rppoptr(c_equal))
		{
			return proc_func_v(sh,tfi,result,vdec,level,v.sub(2));
		}
		if(v.get_left()==rppoptr(c_mbk_l))
		{
			if(v.get(1)==rppoptr(c_addr)&&v.count()==7)
			{
	...
	static rbool is_call(tsh& sh,const rbuf<rstr>& v)
	{
		...
		return v.get(0)=="["&&(v.get(1)==rppkey(c_pcall)||v.get(1)==rppoptr(c_dot));
	}
	...
	static rbool ret_var_proc_one(tsh& sh,rbuf<tword>& v,int& id,tfunc& tfi)
	{
		for(int i=0;i<v.count();i++)
		{
			if(v[i]!=rppoptr(c_mbk_l))
			{
				continue;
			}
			rstr type;
			if(v.get(i+1).val==rppkey(c_pcall))
			{
				type=v.get(i+3).val;
			}
			else
			...
			vword+=name;
			vword+=rppoptr(c_equal);
			vword+=v.sub(i,right+1);
			vword+=rppoptr(c_semi);
			if(i!=0)
			{
				vword+=v.sub(0,i);
				vword+=name;
			...
			vword+=rppoptr(c_semi);
			vword+=rppoptr(c_mbk_l);
			vword+=type;
			vword+=rppoptr(c_comma);
			vword+="~"+type+"("+type+"&)";
			vword+=rppoptr(c_comma);
			vword+=rppoptr(c_mbk_l);
			vword+=name;
			vword+=rppoptr(c_mbk_r);
			vword+=rppoptr(c_mbk_r);
			v=r_move(vword);
			return true;
		}
